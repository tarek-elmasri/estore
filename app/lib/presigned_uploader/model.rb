module PresignedUploader::Model
  ...
  class_methods do
    define_method(:validate_attachment) do |field , maximum_file_size: , accepted_content_types: |
    ...
    define_method(:has_one_file) do |field_name, file_size: , content_type: |
    ...
    define_method(:has_many_files) do |field_name, file_size: , content_type: |
      has_many_attached field_name
      const_set "#{field_name.upcase}_MAXIMUM_FILE_SIZE", file_size
      const_set "#{field_name.upcase}_ACCEPTED_CONTENT_TYPES", content_type
    end
